	...
	public Todo todoDetailView(Connection conn, int todoNo) throws Exception {
		...
		try {
			...
			if (rs.next()) {
				...
				todo = Todo.builder().todoNo(rs.getInt("TODO_NO")).title(rs.getString("TODO_TITLE"))
						.detail(rs.getString("TODO_DETAIL")).complete(complete).regDate(rs.getString("REG_DATE"))
						.build();
